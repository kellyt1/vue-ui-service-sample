...
public class KeycloakSecurityConfig extends KeycloakWebSecurityConfigurerAdapter {
    ...
    /**
     * Use properties in application-{profile}.yml instead of keycloak.json
     */
    @Bean
    @Primary
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        ExpressionUrlAuthorizationConfigurer<HttpSecurity>.ExpressionInterceptUrlRegistry expressionInterceptUrlRegistry = http.cors() //
                .and()
                .csrf().disable()
                // depends on use cases .anonymous().disable()
                .sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS)
                .and()
                .authorizeRequests();

        expressionInterceptUrlRegistry = expressionInterceptUrlRegistry.antMatchers("/test/accounts/rolea*").hasRole("ROLEA");
        expressionInterceptUrlRegistry = expressionInterceptUrlRegistry.antMatchers("/test/accounts/roleb*").hasRole("ROLEB");

        // depends on use cases, probably better to use .denyAll() in most use cases.
